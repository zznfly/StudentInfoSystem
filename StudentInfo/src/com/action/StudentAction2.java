...
public class StudentAction2 extends ActionSupport implements ServletRequestAware{
	...
	public String list(){
		...
		try{
			...
			navCode=NavUtil2.getNavgation("学生信息管理", "学生信息查询");
			total=studentDao.studentCount(con, s_student);
			pageCode=PageUtil.genPagation(request.getContextPath()+"/student2!list", total, Integer.parseInt(page), Integer.parseInt(PropertiesUtil.getValue("pageSize")));
			...
			mainPage="student/studentList2.jsp";
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			try {
